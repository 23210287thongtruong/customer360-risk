...
class SparkIngestionETL:
    def __init__(self, postgres_url, postgres_properties):
        self.spark = (
            SparkSession.builder.appName("Customer360-Spark-Ingestion")
    ...
    def ingest_customers(self, csv_path):
        ...
        df_audited = (
            df_clean.withColumn("ingestion_timestamp", current_timestamp())
        ...
        df_validated = self.validate_dataframe(
            df_audited, "customers", required_columns
        )

        # Write to PostgreSQL staging with truncate (handles deterministic IDs)
        df_validated.write.format("jdbc").option("url", self.postgres_url).option(
            "dbtable", "staging.customers"
        ).option("user", self.postgres_properties["user"]).option(
            "password", self.postgres_properties["password"]
        ).option(
            "driver", self.postgres_properties["driver"]
        ).option(
            "truncate", "true"
        ).mode(
            "overwrite"
    ...
    def ingest_transactions(self, csv_path):
        ...
        # Add audit columns
        df_audited = df_clean.withColumn(
        ...
        df_validated = self.validate_dataframe(
            df_audited, "transactions", required_columns
        )

        # Write to staging
        df_validated.write.format("jdbc").option("url", self.postgres_url).option(
            "dbtable", "staging.transactions"
        ).option("user", self.postgres_properties["user"]).option(
            "password", self.postgres_properties["password"]
        ).option(
            "driver", self.postgres_properties["driver"]
    ...
    def ingest_credit_scores(self, csv_path):
        ...
        if invalid_count > 0:
            logger.warning(
                f"Filtered out {invalid_count} records with invalid credit scores"
            )

        # Add audit columns
        df_audited = df_validated_scores.withColumn(
            "ingestion_timestamp", current_timestamp()
        ).withColumn("source_file", lit(csv_path))

        # Validate
        required_columns = ["customer_id", "credit_score"]
        df_final = self.validate_dataframe(
            df_audited, "credit_scores", required_columns
        )

        # Write to staging
        df_final.write.format("jdbc").option("url", self.postgres_url).option(
            "dbtable", "staging.credit_scores"
        ).option("user", self.postgres_properties["user"]).option(
            "password", self.postgres_properties["password"]
        ).option(
            "driver", self.postgres_properties["driver"]
